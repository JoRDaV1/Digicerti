...
contract Certificate{
    ...
    function addCertificate(uint id, string memory issuedTo, string memory issuer, string memory course, string memory issuedOn) external onlyOwner notAssigned(id) {
        
        allCertificates[id].certificate_hash = keccak256(abi.encodePacked(issuedTo,issuer,course,issuedOn));
